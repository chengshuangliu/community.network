...
#
# Module to send banner commands to maipu Switches
...
DOCUMENTATION = '''
---
module: mpos_interface
short_description: Manage Layer-2 interface on maipu MPOS devices.
description:
...
options:
  ...
  native_vlan:
    description:
      - PVID to be configured in trunk port.
        If C(mode=trunk), used as the trunk PVID .
  trunk_allowed_vlans:
    description:
  ...
  provider:
    description:
      ...
      - For more information please see the
        L(MPOS Platform Options guide, ../network/user_guide/platform_cnos.html).
      - HORIZONTALLINE
...
- name: Ensure Ethernet1/5 is in its default l2 interface state
  community.network.mpos_interface:
    name: Ethernet1/5
    state: unconfigured
...
from ansible_collections.community.network.plugins.module_utils.network.mpos.mpos import load_config
from ansible_collections.community.network.plugins.module_utils.network.mpos.mpos import cnos_argument_spec
from ansible_collections.community.network.plugins.module_utils.network.mpos.mpos import run_commands


def get_interface_type(interface):
    intf_type = 'unknown'
    if interface.upper()[:2] in ('GI', 'FA', 'TE', 'FORTYGI'):
        intf_type = 'ethernet'
    elif interface.upper().startswith('VL'):
        intf_type = 'svi'
    elif interface.upper().startswith('LO'):
        intf_type = 'loopback'
    elif interface.upper()[:2] in ('DC',):
        intf_type = 'management'
    elif interface.upper().startswith('link-aggregation'):
        intf_type = 'link-aggregation'
    return intf_type
...
def is_switchport(name, module):
    ...
    if intf_type in ('ethernet', 'link-aggregation'):
...
def interface_is_portchannel(name, module):
    if get_interface_type(name) == 'ethernet':
        ...
        if any(c in config for c in ['link-aggregation']):
            return True
    return False
...
def get_switchport(name, module):
    ...
    pvid = re.search(r'Default/PVID     : (\d+)', config)
...
def remove_switchport_config_commands(name, existing, proposed, module):
    ...
    if mode == 'access':
        ...
        native_check = existing.get('pvid') == proposed.get('pvid')
        if native_check and proposed.get('pvid'):
            command = 'no switchport trunk pvid'
            commands.append(command)
...
def get_switchport_config_commands(name, existing, proposed, module):
    ...
    if proposed_mode == 'access':
        ...
        native_check = str(existing.get('pvid')) == str(proposed.get('pvid'))
        if not native_check and proposed.get('pvid'):
            command = 'switchport trunk pvid {0}'
            command = command.format(proposed.get('pvid'))
            commands.append(command)
...
def default_switchport_config(name):
    ...
    commands.append('switchport trunk pvid 1')
